...
class DB_Con:
    ...
    def insert_data(self, data, player, strength):
        ...
        for c in data:
            ...
            if prev_value != False:
                # update value
                self.c.execute("UPDATE opponent_modeling SET strength = ((strength+(?-strength))/(num_vals+1)), num_vals = (num_vals+1) WHERE context = ? AND player = ? AND action = ?", (strength, c[0], player, c[1]))
                pass
            else: # Insert new
